...
def to_json(obj_for_json):
    ...
    while i < len(patterns):
        json_s = re.sub(patterns[i], replacements[i], json_s)
        i += 1
...
def main():
    # swapping all string in json-str on SUB
    pattern = r"('(.)+')"
    pattern = r"('[\w]+')"
    s = "[{'asd': 34, 234: [23, 4530.456, 'dsdf'], False: True, None: ('sdf', 0, 324, 'sdf')}, 234, 'sdf', " \
        "{456: 234.01265, 'sdf': 'sdf'}]"
    repl = "SUB"
    print(s)

    swapped_s = re.sub(pattern, repl, s)
    print(swapped_s)

    swaps = re.findall(pattern, s)
    print(swaps)

    print(re.sub(repl, i, swapped_s) for i in swaps)

    for swap in swaps:
        sub = re.search(repl, swapped_s)
        swapped_s = swapped_s[:sub.start()] + swap + swapped_s[sub.end():]
    ...
    var = [{"'as'd": 34, 234: [23, 4530.456, "ds'd'f"], False: True, None: ("s\"df", 0, 324, "s\"d\"f")}, 234, "'s\"df",
           {456: 234.01265, "sdf": "sdf"}]
